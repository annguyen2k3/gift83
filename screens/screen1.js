...
const COLORS = ['#ff6eb4', '#e879f9', '#c084fc', '#ffffff', '#fecdd3', '#f472b6', '#ddd6fe'];
...
export function initScreen1(container, onComplete) {
  ...
  container.innerHTML = `
    ...
    <div class="s1-content">
      ...
      <p class="s1-tagline" id="s1-tagline">Có một điều muốn gửi đến bạn...</p>
  ...
  function mkParticle(targetX, targetY) {
    ...
    return {
      ...
      vx:    fromBtn ? (Math.random() - 0.5) * 3.5 : (Math.random() - 0.5) * 0.45,
      vy:    fromBtn ? (Math.random() - 0.5) * 3.5 - 1 : (Math.random() - 0.5) * 0.45,
  ...
  function drawParticles(delta) {
    ...
    for (const p of bgParticles) {
      ...
      if (p.alpha > 0.75)  { p.alpha = 0.75;  p.aDir = -1; }
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      ...
      setTimeout(() => container.querySelector('#s1-tagline').classList.add('show'), 300);
      setTimeout(() => container.querySelector('#s1-hold-area').classList.add('show'), 700);
      setTimeout(() => container.querySelector('#s1-label').classList.add('show'), 1100);
      setTimeout(() => container.querySelector('#s1-dots').classList.add('show'), 1300);
    });
  });
